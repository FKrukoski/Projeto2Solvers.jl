...
"""
    l_bfgs_rcst(nlp; options...)

Este método é chamado L-BFGS com região de confiança por Steihaug-Toint.

Tenta-se resolver B_k d = -\nabla f(x_k) usando Gradientes Conjugados. 
Se em algum momento a direção ficar maior que a região de confiança, 
ela é truncada. B_k é a aproximação de memória limitada de BFGS. 
Porém, deve-se guardar apenas os últimos p vetores s_k e y_k, 
e não criar a matriz B_k explicitamente. 
